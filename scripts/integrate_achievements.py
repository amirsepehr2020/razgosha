...
achievements = '''const ACHIEVEMENTS = [
  ...
  ["evidence-reader", "🔎 ردیاب شواهد", "۱۰ سرنخ از پرونده‌های حل‌شده جمع کن"],
  ["multi-stage", "🧩 استاد مراحل", "۵ پرونده چندمرحله‌ای رو حل کن"],
...
help_text = '''  achievements: `🏅 دستاوردها\\n\\nمسیر کارآگاهیت اینجاست.\\n\\n🟢 شروع: اولین، سومین و پنجمین پرونده\\n🔵 پیشرفت: ۱۰، ۲۰ و ۳۰ پرونده\\n🟣 حرفه‌ای: ۵۰ و ۶۰ پرونده\\n🟠 مهارت: ردیاب شواهد و استاد مراحل\\n🔴 مخفی: چشم عقاب، ردیاب زمان، ذهن پشت پرده و غیرممکن؟\\n\\n📊 برای دستاوردهای قابل‌نمایش، پیشرفتت به شکل x/y دیده می‌شه.\\n🔒 دستاوردهای مخفی تا زمان کشف، هویت و شرطشون رو نشون نمی‌دن.\\n🎁 دستاوردهای جدید می‌تونن جایزه امتیازی داشته باشن.`,'''
...
unlocker = '''async function unlockAchievements(env, playerId, chatId = null) {
  ...
  const evidenceCount = solvedCases.reduce((sum, c) => sum + (Array.isArray(c.clues) ? c.clues.length : 0), 0);
  ...
  if (evidenceCount >= 10) wanted.push("evidence-reader");
  if (multiStageCount >= 5) wanted.push("multi-stage");
  ...
  const rewards = {
    "first-case": 10, "three-cases": 15, "five-cases": 20, "ten-cases": 30,
    "twenty-cases": 40, "thirty-cases": 50, "fifty-cases": 75, "sixty-cases": 100,
    "evidence-reader": 15, "multi-stage": 25, "perfect-streak": 50,
  ...
  for (const id of newlyUnlocked) {
    ...
    if (reward) {
      await env.DB.prepare("UPDATE players SET score=score+?, updated_at=? WHERE id=?").bind(reward, new Date().toISOString(), playerId).run();
    }
  }

  if (chatId && newlyUnlocked.length) {
    const lines = newlyUnlocked.map(id => {
      const item = ACHIEVEMENTS.find(a => a[0] === id);
      return `🏅 ${item?.[1] || id}\\n🎁 +${rewards[id] || 0} امتیاز`;
    }).join("\\n\\n");
    await sendMessage(env, chatId, `🎉 دستاورد جدید باز شد!\\n\\n${lines}\\n\\nادامه بده کارآگاه؛ نشان‌های مخفی هنوز منتظر کشف شدنن 👀`, MENU, 700, "🏅");
  }
...
viewer = '''async function achievements(env, chatId, player) {
  ...
  const multiStageCount = solvedCases.filter(c => getStageCount(c) >= 3).length;
  const progress = {
    "first-case": [solved, 1], "three-cases": [solved, 3], "five-cases": [solved, 5],
    "ten-cases": [solved, 10], "twenty-cases": [solved, 20], "thirty-cases": [solved, 30],
    "fifty-cases": [solved, 50], "sixty-cases": [solved, 60], "evidence-reader": [evidenceCount, 10],
    "multi-stage": [multiStageCount, 5]
  };
  const hidden = new Set(["eagle-eye", "time-tracker", "mastermind", "impossible", "perfect-streak"]);
  ...
  const text = [
    "🏅 دستاوردها",
    "",
    `📊 ${unlockedCount}/${ACHIEVEMENTS.length} نشان باز شده`,
    "",
    "🟢 شروع",
    ...visible.slice(0, 2),
    "",
